...
//[{"title":"Redmi+Note+9+Pro+(Glacier+White,+4GB+RAM,+64GB+Storage)","Colour":"Champagne+Gold","Size":"Medium"}]
            if($var_id_jason == '0'){
                ...
                if($product_obj){
                    ...
                    $product_net_revenue = ($product_obj->sale_price) ? $product_obj->sale_price : $product_obj->regular_price;
                }
		...
		## if user loging
		 if($this->LOGIN_ID){
			...
			if(!empty($item_obj) && $item_obj->order_item_name==$product_name){
				...
				$item_arr = $this->Query_model->get_data_obj('ec_order_product',array('customer_id' => $this->LOGIN_ID, 'order_item_id' => $item_obj->order_item_id,'order_id' => NULL));
                $item_arr_product_qty = isset($item_arr->product_qty) ? $item_arr->product_qty : 0;
				
				$qty = $item_arr_product_qty + $product_qty;
				$db_data1 = array(
					'product_qty'       		=> $qty,
			...
			//print_r($item_obj);	print "$product_name";		
			if(!empty($item_obj) && $item_obj->order_item_name==$product_name){
				$get_cart = get_cart();
				
				#ec_order_product tables update				
				$item_arr = $this->Query_model->get_data_obj('ec_order_product',array('session_id' => $session_id, 'order_item_id' => $item_obj->order_item_id,'order_id' => NULL));
			    $item_arr_product_qty = isset($item_arr->product_qty) ? $item_arr->product_qty : 0;
				$qty = $item_arr_product_qty + $product_qty;
				$db_data1 = array(
					'product_qty'       => $qty,
    ...
    {
		$cart_details = $this->session->userdata('cart');
		//print '<pre>';print_r($cart_details);
		$data['csrf'] = csrf_token();
		...
		if(isset($cart_details) && isset($cart_details[$order_itemid])){
			$cart_details[$order_itemid]['product_qty'] = $order_quantity;
			$this->session->set_userdata('cart', $cart_details);
		}
		//exit;
		$data['csrf'] = csrf_token();
        $data = array();
        //$delete = $this->Product_model->update_cart($order_item_id);
		$data = array(
			'product_qty' => $order_quantity,
			//'order_id'    =>	
		);		
		$update_cartqty = $this->Query_model->update_data('ec_order_product',$data,array('order_item_id' => $order_itemid));
